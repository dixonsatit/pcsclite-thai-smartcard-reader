...
class ThaiCardReader extends EventEmitter {
  ...
  onReader(reader) {
    ...
    this.emit(EVENTS.DEVICE_CONNECTED, reader)
    ...
    reader.on("status", (status) => {
      ...
      if (changes & reader.SCARD_STATE_EMPTY && status.state & reader.SCARD_STATE_EMPTY) {
        // Card remove
        this.emit(EVENTS.CARD_REMOVED, reader)
        ...
        this.emit(EVENTS.CARD_INSERTED, reader)
        setTimeout(() => {
          // Start reading data
          ...
          reader.connect({ share_mode: reader.SCARD_SHARE_SHARED }, (err, protocol) => {
            ...
            readData(reader, protocol, this.readMode === MODE.PERSONAL_PHOTO, (res) => {
              switch (res.status) {
                case STATUS.START:
                  this.emit(EVENTS.READING_START, reader)
                  break
                case STATUS.READING:
    ...
    reader.on("end", () => {
      ...
      this.emit(EVENTS.DEVICE_DISCONNECTED, reader)
    })
  }
}
